...
- `title` — short human title.
- `status` — optional on create (defaults to `active`); may be set explicitly.
- Type-specific content fields (kept minimal; identified in implementation).

Recommended (not enforced) create input: `summary`, `author`, `tags`, `refs`.

**Stored/output envelope (full record returned by reads/repacks):**
`id`, `type`, `project`, `title`, `status`, `created_at`, `updated_at`,
`summary`, `author`, `tags`, `refs`, `schema_version`, plus type-specific
fields. This is the full common envelope above; it always includes the
generated fields because Zentext assigned them on create.
